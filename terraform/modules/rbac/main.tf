...
resource "azurerm_role_assignment" "eventgrid_to_servicebus" {
  principal_id         = var.eventgrid_system_topic_principal_id
  scope                = var.servicebus_namespace_id
  role_definition_name = "Azure Service Bus Data Sender"
}

resource "azurerm_role_assignment" "batch_ingest_service_acr" {
...
resource "azurerm_role_assignment" "batch_ingest_service_blob" {
  principal_id         = var.batch_ingest_app_principal_id
  scope                = var.storage_account_id
  role_definition_name = "Storage Blob Data Contributor"
}

resource "azurerm_cosmosdb_sql_role_assignment" "batch_ingest_service_cosmos" {
  resource_group_name = var.resource_group_name
  account_name        = var.cosmos_account_name
  scope               = var.cosmos_account_id
  role_definition_id  = "${var.cosmos_account_id}/sqlRoleDefinitions/00000000-0000-0000-0000-000000000002"
